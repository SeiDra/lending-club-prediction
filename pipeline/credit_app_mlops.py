...
def load_artifacts():
    mp, cp = os.path.join(MODEL_DIR, "best_model.pkl"), os.path.join(
        MODEL_DIR, "features_config.json"
    )
...
LABELS = {
    ...
    "dti": "Ratio dettes/revenus (DTI)",
    "annual_inc": "Revenu annuel ($)",
    "loan_amnt": "Montant du prêt ($)",
    "installment": "Mensualité ($)",
    "revol_bal": "Solde revolving ($)",
    "revol_util": "Utilisation revolving (%)",
    "loan_to_income": "Ratio prêt/revenu",
    "installment_to_income": "Ratio mensualité/revenu",
    "fico_avg": "Score FICO moyen",
    "emp_length_yrs": "Ancienneté (ans)",
    "open_acc": "Comptes ouverts",
    "total_acc": "Total comptes",
    "all_util": "Utilisation globale (%)",
    "il_util": "Util. prêts tempérament (%)",
    "bc_util": "Util. bancaire (%)",
    "total_bc_limit": "Limite crédit bancaire ($)",
    "total_derog_signals": "Signaux négatifs (0-4)",
    "mths_since_last_record": "Mois depuis incident",
    "mths_since_recent_bc_dlq": "Mois depuis défaut BC",
    "mths_since_recent_bc": "Mois depuis BC récente",
    "pct_tl_nvr_dlq": "% comptes sans défaut",
    "acc_open_past_24mths": "Comptes ouverts (24 mois)",
    "mo_sin_old_il_acct": "Âge + ancien compte (mois)",
    "mo_sin_old_rev_tl_op": "Âge + ancienne ligne rev. (mois)",
    "credit_age_years": "Âge crédit (ans)",
    "bc_open_to_buy": "Crédit bancaire disponible ($)",
    "has_delinq": "Retards passés",
    "has_pub_rec": "Registre public",
    "has_bankruptcy": "Faillite déclarée",
    "term_ 60 months": "Durée 60 mois",
}
...
def fmt(f, v, stats):
    ...
    if any(k in f for k in ["amnt", "inc", "bal", "limit", "buy"]):
        return f"${v:,.0f}"
    if any(k in f for k in ["rate", "util", "dti", "pct"]):
        return f"{v:.1f}%"
    if "mths" in f or "mo_sin" in f:
        return f"{v:.0f} mois"
    return f"{v:.2f}"
...
PROFILES = {
    ...
    "✅ Profil Faible Risque": {
        ...
        "emp_length_yrs": 8,
        "loan_to_income": 0.07,
        "installment_to_income": 0.027,
        "all_util": 18.0,
        "bc_util": 12.0,
        "total_bc_limit": 60000,
        "has_delinq": 0,
        "term_ 60 months": 0,
        "total_derog_signals": 0,
        "pct_tl_nvr_dlq": 98.0,
        "mo_sin_old_il_acct": 120,
        "mo_sin_old_rev_tl_op": 180,
        "bc_open_to_buy": 45000,
    ...
    "⚠️  Profil Risque Modéré": {
        ...
        "emp_length_yrs": 3,
        "loan_to_income": 0.33,
        "installment_to_income": 0.105,
        "all_util": 58.0,
        "bc_util": 52.0,
        "total_bc_limit": 25000,
        "has_delinq": 0,
        "term_ 60 months": 1,
        "total_derog_signals": 1,
        "pct_tl_nvr_dlq": 80.0,
    ...
    "❌ Profil Haut Risque": {
        ...
        "emp_length_yrs": 1,
        "loan_to_income": 0.94,
        "installment_to_income": 0.319,
        "all_util": 85.0,
        "bc_util": 90.0,
        "total_bc_limit": 10000,
        "has_delinq": 1,
        "term_ 60 months": 1,
        "total_derog_signals": 3,
        "pct_tl_nvr_dlq": 45.0,
        "mo_sin_old_il_acct": 18,
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='color:#5577aa;font-size:.8rem'>LendingClub · Modèle LightGBM</div>",
        unsafe_allow_html=True,
    )
        ...
        for gname, gfeats in grouped.items():
            ...
            for feat in gfeats:
                if feat not in stats:
                    continue
                s = stats[feat]
                is_bin = s.get("is_binary", False)
                fmin, fmax, fmed = float(s["min"]), float(s["max"]), float(s["median"])
                default = float(
                    np.clip(
                        float(prof_vals.get(feat, fmed)) if prof_vals else fmed,
                        fmin,
                        fmax,
                    )
                )
                if is_bin:
                    val = st.selectbox(
                        ...
                        format_func=lambda x: "Oui (1)" if x else "Non (0)",
                        key=f"feat_{feat}",
                    )
                else:
                    r = fmax - fmin
...
with col_t:
    st.markdown(
        '<div class="main-title">Analyse de Risque de Crédit</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="main-sub">LendingClub · Prédiction de défaut · LightGBM optimisé via MLflow</div>',
        unsafe_allow_html=True,
    )
st.markdown("<hr/>", unsafe_allow_html=True)
...
rc = lambda p: "#00c853" if p < 0.35 else ("#ffd600" if p < 0.60 else "#ff1744")
badge = lambda p: (
    '<span class="ba">✅ APPROUVÉ</span>'
    if p < 0.35
    else (
        '<span class="bw">⚠️ EXAMEN REQUIS</span>'
        if p < 0.60
        else '<span class="br">❌ REFUSÉ</span>'
    )
)
dtxt = lambda p: (
    "Profil à faible risque. Prêt recommandé."
    if p < 0.35
    else (
        "Profil à risque modéré. Vérification manuelle conseillée."
        if p < 0.60
        else "Profil à risque élevé de défaut. Prêt déconseillé."
    )
)

proba, pred = (
    standardize_and_predict(input_values, config["features"], config["stats"])
    if input_values
    else (0.0, 0)
)
c = rc(proba)
...
with c1:
    st.markdown(
        f'<div class="mcard"><div class="lbl">Probabilité de Défaut</div><div class="val" style="color:{c}">{proba:.1%}</div></div>',
        unsafe_allow_html=True,
    )
with c2:
    score = max(0, min(1000, int((1 - proba) * 1000)))
    st.markdown(
        f'<div class="mcard"><div class="lbl">Score Interne (0–1000)</div><div class="val" style="color:{c}">{score}</div></div>',
        unsafe_allow_html=True,
    )
with c3:
    niv = "Faible" if proba < 0.35 else ("Modéré" if proba < 0.60 else "Élevé")
    st.markdown(
        f'<div class="mcard"><div class="lbl">Niveau de Risque</div><div class="val" style="color:{c};font-size:1.4rem">{niv}</div></div>',
        unsafe_allow_html=True,
    )
with c4:
    st.markdown(
        f'<div class="mcard"><div class="lbl">Décision</div><div style="margin-top:.6rem">{badge(proba)}</div></div>',
        unsafe_allow_html=True,
    )
...
with cg:
    fig = go.Figure(
        go.Indicator(
            ...
            number={
                "suffix": "%",
                "font": {"size": 38, "color": c, "family": "DM Mono"},
            },
            gauge={
                "axis": {
                    "range": [0, 100],
            ...
            title={
                "text": "Risque de Défaut",
                "font": {"size": 14, "color": "#5577aa"},
            },
            domain={"x": [0, 1], "y": [0, 1]},
        )
    )
    ...
    st.markdown(
        f"<p style='text-align:center;color:#8899bb;font-size:.85rem'>{dtxt(proba)}</p>",
        unsafe_allow_html=True,
    )

with ci:
    st.markdown(
        "<p style='color:#5577aa;font-size:.75rem;letter-spacing:.1em;text-transform:uppercase;font-weight:600'>Top 10 — Importance des Features</p>",
        unsafe_allow_html=True,
    )
    try:
        imps = (
            pd.Series(model.feature_importances_, index=config["features"])
...
st.markdown("<hr/>", unsafe_allow_html=True)
st.markdown(
    "<p style='color:#5577aa;font-size:.75rem;letter-spacing:.1em;text-transform:uppercase;font-weight:600'>Valeurs du Profil Analysé</p>",
    unsafe_allow_html=True,
)
if input_values:
    ...
    for i, col in enumerate(cols):
        with col:
            for l, vs in items[i * chunk : (i + 1) * chunk]:
                st.markdown(
                    f"<div style='display:flex;justify-content:space-between;border-bottom:1px solid #1e2d4a;padding:.3rem 0;'><span style='color:#5577aa;font-size:.78rem'>{l}</span><span style='color:#c8d8f0;font-family:DM Mono,monospace;font-size:.78rem'>{vs}</span></div>",
                    unsafe_allow_html=True,
                )

st.markdown("<hr/>", unsafe_allow_html=True)
st.markdown(
    "<p style='text-align:center;color:#2a3a5a;font-size:.75rem'>Projet MLOps — LendingClub · LightGBM · MLflow · Streamlit</p>",
    unsafe_allow_html=True,
)
